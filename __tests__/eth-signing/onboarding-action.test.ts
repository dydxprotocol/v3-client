  ...
  describe('without a web3 provider', () => {
    ...
    it('signs and verifies using SigningMethod.Hash', async () => {
      const signature = await localSigner.sign(
        localAccountAddress,
        SigningMethod.Hash,
        { action: OnboardingActionString.ONBOARDING, domain: 'https://trade.dydx.exchange' },
      );
      expect(
        localSigner.verify(
          signature,
    ...
    it('rejects if the message is different', async () => {
      ...
      expect(
        localSigner.verify(
          signature,
          localAccountAddress,
          { action: OnboardingActionString.KEY_DERIVATION, domain: 'https://trade.dydx.exchange' },
        ),
      ).toBe(false);
    });
  });
